...
- La soppressione via API **non funziona attraverso il proxy di Unsloth Studio**: `reasoning_effort:none`, `chat_template_kwargs:{enable_thinking:false}`, `reasoning:{enabled:false}`, `/no_think`, istruzioni esplicite nel system → sempre ~480-560 token, ~20-26 s (testate tutte, `scratchpad/test_nothink.py`). ⚠ **Aggiornamento 2026-07-15**: bypassando il proxy e parlando direttamente con llama-server (`--reasoning off`), il CoT **è soppresso completamente** (3.4 s/paragrafo vs 29.7 s; pagina densa ~21 s) — il limite era del proxy, non del modello. Vedi [llamacpp-direct-wayfinder.md](./llamacpp-direct-wayfinder.md).
